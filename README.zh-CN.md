...
- **按日期的推荐过滤** — `getRecommendedSessions` 在传入 ISO 日期时可只显示该日的 mock 会话
...
### Google 集成
- **Google 登录** — 配置了 `NEXT_PUBLIC_GOOGLE_CLIENT_ID` 时使用 OAuth；未配置时 Google 按钮走**演示登录**（不挂载空的 OAuth Provider，避免在 GitHub Pages 等静态环境白屏）
...
StudyLink/
├── .github/workflows/
│   └── deploy-github-pages.yml    # 静态导出并部署到 GitHub Pages
├── public/
│   ├── .nojekyll                  # 防止 GitHub Pages 的 Jekyll 忽略 `/_next` 目录
...
│   │   ├── GoogleOAuthWrapper.tsx # 仅在配置了 Client ID 时挂载 Google OAuth Provider
...
│       ├── mock-data.ts           # 约 20 名 mock 学生、26 个 mock 会话、匹配辅助函数
│       ├── auth.ts                # 基于 CSV 的认证
│       ├── public-path.ts         # 带 basePath 的 `public/` 资源 URL（GitHub Pages）
...
├── next.config.js                 # 静态导出（`output: 'export'`），可选 basePath 用于 Pages
├── package.json                   # 依赖和脚本
...
`RouteGuard` 会处理 `NEXT_PUBLIC_BASE_PATH` 与末尾斜杠，使 `/login` 等公开路由在 `https://user.github.io/repo/login/` 下仍被识别为登录页。
...
未配置 Google OAuth 时，可在登录页使用 **Sign in with Google (Demo)** 一键进入演示账号流程。
...
3. **公开路由**（`/login`、`/`）跳过所有检查
4. **子路径部署** — 去掉 `NEXT_PUBLIC_BASE_PATH` 与末尾斜杠，使 `/repo/login/` 与 `/login` 等价
